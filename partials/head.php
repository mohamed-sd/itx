<?php
/* Shared <head> resources. Page-specific <title>, description, canonical,
   Open Graph and JSON-LD stay in each page (they are DB-driven). Include
   this near the top of <head>; charset/viewport come first. */
?>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<meta name="theme-color" content="#0E2150">
<link rel="icon" type="image/png" href="assets/itx-mark.png">
<link rel="preconnect" href="https://fonts.googleapis.com">
<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
<link href="https://fonts.googleapis.com/css2?family=IBM+Plex+Sans+Arabic:wght@400;500;600;700&family=Space+Grotesk:wght@500;600;700&display=swap" rel="stylesheet">
<link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css" rel="stylesheet">
<link rel="stylesheet" href="assets/css/styles.css?v=<?= @filemtime(__DIR__ . '/../assets/css/styles.css') ?>">
<link rel="stylesheet" href="assets/css/app.css?v=<?= @filemtime(__DIR__ . '/../assets/css/app.css') ?>">
